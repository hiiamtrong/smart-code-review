...
print_next_steps() {
  ...
  echo "  5. (Optional) Per-project config overrides (inside a git repo):"
  echo "       ai-review config set --project AI_MODEL          gpt-4o"
  echo "       ai-review config set --project AI_PROVIDER        openai"
  echo "       ai-review config set --project ENABLE_AI_REVIEW   false"
  echo "  6. cd into any git repo"
  echo "  7. Run: ai-review install     — install the pre-commit hook"
  echo ""
  echo "Commands:"
