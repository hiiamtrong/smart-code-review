...
func TestAutoGenerateProperties_createsFile(t *testing.T) {
	dir := t.TempDir()
	path, err := AutoGenerateProperties(dir, "my-project")
	if err != nil {
...
func TestAutoGenerateProperties_existingFileNotOverwritten(t *testing.T) {
	dir := t.TempDir()
	propsPath := filepath.Join(dir, "sonar-project.properties")
	original := "sonar.projectKey=existing\n"
	os.WriteFile(propsPath, []byte(original), 0644)

	path, err := AutoGenerateProperties(dir, "new-key")
	if err != nil {
...
func TestAutoGenerateProperties_sanitizesEmptyKey(t *testing.T) {
	...
	path, err := AutoGenerateProperties(dir, "")
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	data, _ := os.ReadFile(path)
...
func TestAutoGenerateProperties_jsProject(t *testing.T) {
	...
	_, err := AutoGenerateProperties(dir, "js-proj")
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	data, _ := os.ReadFile(filepath.Join(dir, "sonar-project.properties"))
...
func TestAutoGenerateProperties_goProject(t *testing.T) {
	...
	_, err := AutoGenerateProperties(dir, "go-proj")
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	data, _ := os.ReadFile(filepath.Join(dir, "sonar-project.properties"))
...
func TestWaitForTask_taskSucceeds(t *testing.T) {
	...
	scannerDir := filepath.Join(dir, ".scannerwork")
	os.MkdirAll(scannerDir, 0755)
	os.WriteFile(filepath.Join(scannerDir, "report-task.txt"),
